...
vorbis_info_floor* floor1_info_unpack(vorbis_info *vi, oggpack_buffer *opb) {
	...
	vorbis_info_floor1 *info = (vorbis_info_floor1*) _ogg_calloc(1,
			sizeof(*info));
	/* read partitions */
	info->partitions = oggpack_read(opb, 5); /* only 0 to 31 legal */
	info->partitionclass = (char*) _ogg_malloc(
	...
	info->class = (floor1class*) _ogg_malloc(
			(maxclass + 1) * sizeof(*info->class));
	...
	info->postlist = (ogg_uint16_t*) _ogg_malloc(
			(count + 2) * sizeof(*info->postlist));
	info->forward_index = (char*) _ogg_malloc(
		...
		for (i = 0, j = 2; i < info->partitions; i++) {
			...
			if (csubbits) {
				cval = vorbis_book_decode(
						books + info->class[classv].class_book, &vd->opb);
